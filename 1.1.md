...
我们可以想一下，大多数涉及到列表操作的业务场景都是从列表里边删除某一项，或者从列表里边插入某一项。针对这类大多数的情况我们就可以优化这里的diff算法。为了简述方便，我们用字母 a, b, c 来表示 VNode 节点。
...
2.  ```oldEndVnode``` 和 ```newStartVnode``` 相同
...
3.  ```oldStartVnode``` 和 ```newEndVnode``` 相同
   ...
   function updateChildren (parentElm, oldCh, newCh) {
     ...
     if (/* blabla */) {
       ...
       document.insertBefore(parentElm, oldStartVnode.elm, document.nextSibling(oldEndVnode.elm))
       oldStartVnode = oldCh[++oldStartIdx]
       newEndVnode = newCh[--newEndIdx]
     }
   }
...
4.  ```oldEndVnode``` 和 ```newEndVnode```相同
   ...
   function updateChildren (parentElm, oldCh, newCh) {
     ...
     if (/* blabla */) {
       ...
       document.insertBefore(parentElm, oldEndVnode.elm, oldStartVnode.elm)
       oldEndVnode = oldCh[--oldEndIdx]
       newStartVnode = newCh[++newStartIdx]
     }
   }
