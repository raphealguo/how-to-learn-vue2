      ...
      if (template) {
        ...
        if (typeof template === 'string') {
          if (template.charAt(0) === '#') {
            ...
            /* istanbul ignore if */
            if (!template) {
              (0, _index4.warn)('Template element not found or is empty: ' + options.template, this);
            }
          }
        } else if (template.nodeType) {
          template = template.innerHTML;
        } else {
          (0, _index4.warn)('invalid template option:' + template, this);
          return this;
        }
      } else if (el) {
        // 从dom节点里边取
...
function mountComponent(vm, el) {
  ...
  vm._watcher = new _watcher2.default(vm, updateComponent, _index4.noop);
...
function updateChildComponent(vm, propsData) {
  ...
  if (propsData && vm.$options.props) {
    ...
    for (var i = 0; i < propKeys.length; i++) {
      var key = propKeys[i];
      props[key] = (0, _index4.validateProp)(key, vm.$options.props, propsData, vm);
    }
...
function query(el) {
  if (typeof el === 'string') {
    ...
    if (!selected) {
      (0, _index4.warn)('Cannot find element: ' + el);
...
var _createElement = __webpack_require__(32);

var _renderList = __webpack_require__(24);

var _checkKeycodes = __webpack_require__(23);
...
var _textParser = __webpack_require__(19);
...
function createComponent(Ctor, data, context, children, tag) {
  ...
  // 生成一个特殊的vnode 带有
  var vnode = new _vnode2.default('vue-component-' + Ctor.cid + (name ? '-' + name : ''), data, undefined, undefined, undefined, context, { Ctor: Ctor, propsData: propsData, tag: tag, children: children });
  return vnode;
}
...
var _createComponent = __webpack_require__(31);
...
var _domProps = __webpack_require__(33);
